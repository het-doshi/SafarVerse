...
function Post({ title, description, image, email, currentUser, id}) {
  ...
  const Delete = async (id) => {
    try {
        console.log(id)
        const response = await axios.delete(`http://localhost:4000/api/deleteTip?id=${id}`); 
  ...
  return (
    <div className="d-flex justify-content-center mt-5">
      <ToastContainer />
      <Card style={{ width: "700px", marginTop: "1px" }}>
        <CardImg className="postImg" alt="Travel Tip" src={image} style={{ width: "100%", height: "260px", borderRadius: "5px 5px 0 0" }} />
        <CardBody>
